...
public:
    ...
    // function overloading: display() with no args shows full details
...
public:
    ...
    // + operator: adds prices of two listings (e.g. bundle deal total)
    double operator+(const Listing& other) const { return price + other.price; }
    ...
    // function overloading: display(bool) shows compact one-line version
    void display(bool compact) {
        if (compact)
            cout << "Listing #" << id << " | " << car.getBrand() << " " << car.getModel() << " | PKR " << price << endl;
        else
            display();
    }
...
int main() {
    ...
    // + operator: total price of two listings combined
    double bundlePrice = l1 + l3;
    cout << "l1 + l3 bundle price = PKR " << bundlePrice << " (Listing +)" << endl;

    // function overloading: display() vs display(true)
    cout << "\nFull display:" << endl;
    l1.display();
    cout << "Compact display:" << endl;
    l1.display(true);
    l2.display(true);
    l3.display(true);
